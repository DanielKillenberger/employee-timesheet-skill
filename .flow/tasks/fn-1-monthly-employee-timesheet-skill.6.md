...
## Done summary
Packaged the repository into an installable Claude skill and made the written guidance carry a non-technical user on its own. `SKILL.md` (name/description within limits, ~2.8k-token body) tells Claude to settle a persistent data folder outside any git worktree first, then drives register / generate / photo→validate→confirm→tally, holding the line that nothing is final before the user confirms and that an unmatched name on the sheet blocks confirmation. `scripts/package_skill.py` builds `employee-timesheet.zip` from an allowlist — an unlisted folder cannot leak into a release by default — with `employee-timesheet/` at the ZIP root; review caught that `is_file()`/`ZipFile.write()` follow symlinks, so a link named `assets/logo.png` could have published the local registry under an innocent name, and collection now refuses symlinks and anything resolving outside the repo. 32 packaging tests read the built ZIP itself (layout, required resources, bundled template validity, forbidden dirs/files/documents/photos, frontmatter limits); 437 tests pass overall.

README was rewritten around a self-contained install guide whose click path was verified against Anthropic's current help article (code execution first, incl. the Team/Enterprise owner step; then Customize → Skills → + → + Create skill → Upload a skill), with `/releases/latest` deep-linked and a privacy section that says honestly where files live per usage mode instead of claiming everything stays on the user's machine. AC11 was covered in three legs: the packaged ZIP installed into a bare folder and driven with plain `python3` — including a real vision pass where a filled sheet image was read back and transcribed from the picture (blank working day and an ambiguous 6-vs-8 both flagged, confirmation refused until answered, tally 159.0 h × 28.50 CHF = 4531.50 CHF); a link-only relay test where a fresh agent given nothing but the public repo URL produced a correct jargon-free install walkthrough; and the user's own live claude.ai upload and conversational pass ("it works fine"). Frictions found were fixed rather than documented around: where to run the script from, where the transcribed-entries JSON belongs, and export/import paths that the git-worktree guard would have rejected.
## Evidence
- Commits: 656c6f2d940cf9976cc18fffcffba038363b75f0, 738fe911713f97509489d57cc6c8e1451c787852, 4a3cba388dc5f1d0396779b0983411445855d3eb, 670158f69a0c30e5715340cb371a1313650de503, 167f31544158186deec8e077c751abeb45aeb342, 44f11d8c1eb779071f16a3943da59e16b183bb9c, 6c92de440cca7d32c8d43c7e353e4e73cd48afcc, 497836f6d164884c410d9eef66f86b62532d53db
- Tests: uv run pytest (437 passed), uv run scripts/package_skill.py --output-dir dist, GATE_SKIPPED:unittest:green-receipt 7fc3d7dc - baseline reused from prior post-gate pass, .flow/bin/flowctl validate --all --json (0 errors, 0 warnings)
- PRs: